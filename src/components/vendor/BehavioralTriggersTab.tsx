      ...
      {couponsWithTriggers.map((c: any) => (
        <Card key={c.id} className="border-slate-200 shadow-sm">
          ...
          <CardContent className="space-y-3 pt-4">
            {c.behavioralTriggers.map((tTrigger: any) => (
              <div
                key={tTrigger.id}
              ...
              >
                <div>
                  <p className="font-bold text-slate-800 capitalize flex items-center gap-1.5 mb-0.5">
                    ...
                    {t('vendor.behavioral.rule_of', 'Regra de')}{' '}
                    {tTrigger.type === 'visit'
                      ? t('vendor.behavioral.visit', 'Visita')
                      : tTrigger.type}
                  </p>
                  <p className="text-slate-500 font-medium">
                    {t('vendor.behavioral.active_every', 'Ativa a cada')}{' '}
                    <strong className="text-slate-700">
                      {tTrigger.threshold}
                    </strong>{' '}
                    {tTrigger.type === 'visit'
                      ? t('vendor.behavioral.visits', 'visitas')
                      : t('vendor.behavioral.actions', 'ações')}
                  </p>
                </div>
                <div className="text-right bg-slate-50 py-2 px-3 rounded-lg border border-slate-100">
                  ...
                  <p className="font-black text-primary">{tTrigger.reward}</p>
                </div>
              </div>
            ))}
          </CardContent>
