...
DECLARE
  new_user_id uuid;
BEGIN
  -- Create the master admin user if it does not exist to prevent access loss
  ...
  ELSE
    -- Ensure existing user has super_admin role in profiles (redundant security layer)
    UPDATE public.profiles 
    SET role = 'super_admin' 
    WHERE email = 'adailtong@gmail.com';
    
    -- Ensure user metadata reflects super_admin
    UPDATE auth.users
