...
class ProfessionalScraper {
  ...
  parseMercadoLivre($: cheerio.CheerioAPI, baseUrl: string): ScrapedItem[] {
    ...
    elements.each((_, el) => {
      ...
      const promoPrice = $(el)
        .find(
          '.ui-search-price__part--primary .andes-money-amount__fraction, .poly-price__current .andes-money-amount__fraction',
        )
        .first()
        .text()
        .trim()
      ...
      // Busca preço antigo
      const strikethroughPrice = $(el)
        .find(
          's.andes-money-amount .andes-money-amount__fraction, .poly-price__original .andes-money-amount__fraction',
        )
        .first()
        .text()
        .trim()
