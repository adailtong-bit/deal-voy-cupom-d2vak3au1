      ...
      $search('.result').each((i, el) => {
        ...
        if (title && rawUrl.startsWith('http')) {
          ...
          items.push({
            ...
            campaign_name: siteDomain ? `Busca na fonte: ${siteDomain}` : 'Busca Multi-fontes Orgânica',
            price: price,
            currency: priceText?.includes('€') ? 'EUR' : priceText?.includes('$') && !priceText.includes('R$') ? 'USD' : 'BRL',
            status: 'pending',
            category: options?.category && options.category !== 'all' ? options.category : 'geral',
    ...
    if (items.length === 0 && targetUrl && targetUrl !== 'all') {
       ...
       try {
         ...
         if (directResp.ok) {
           ...
           items.push({
             title: ($('title').text().trim() || siteDomain || 'Busca Direta').substring(0, 255),
