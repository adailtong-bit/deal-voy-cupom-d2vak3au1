  ...
  try {
    ...
    if (targetUrl) {
      ...
      try {
        ...
        try { rawData['extracted_domain'] = new URL(currentUrl).hostname; } catch(e) {}
        ...
        $('#description, .description, #product-description, .product-description, [data-testid="product-description"], #productDescription').each((i, el) => {
           if (i === 0) {
             const text = $(el).text().trim().replace(/\s+/g, ' ');
             if (text) rawData['detected_description_main'] = text.substring(0, 1000);
           }
        });
        ...
        const priceElements = $('[class*="price" i], [id*="price" i], [data-price]');
        let priceIndex = 1;
