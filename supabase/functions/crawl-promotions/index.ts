...
function extractOfferFields($: cheerio.CheerioAPI, el: any, fallbackUrl: string, sourceName: string) {
  ...
  const ctaMatch = bodyText.match(/comprar|buy now|add to cart|adicionar ao carrinho|shop now|shop sale|get deal|resgatar/i)
    ...
    if (items.length === 0 && targetUrl && targetUrl !== 'all') {
      ...
      try {
        ...
        const directResp = await fetch(directUrl, {
          headers: isAmazon ? amazonHeaders : { ...browserHeaders, 'Referer': `https://${siteDomain}/` },
        ...
        if (directResp.ok) {
          ...
          if (isAmazon) {
            ...
            const itemsList = $('div[data-component-type="s-search-result"], [class*="DealGridItem"], .sg-col-inner, .a-carousel-card')
            
            itemsList.each((i: number, el: any) => {
              if (items.length >= limit) return
              
              const title = $(el).find('span.a-size-base-plus, h2, [class*="dealTitle"], .a-truncate-cut').first().text().trim() || "Sem título"
              const price = $(el).find('span.a-price, span.a-price-whole, [class*="priceBlock"]').first().text().trim() || "Preço não encontrado"
              const oldPrice = $(el).find('span.a-price.a-text-price, [class*="basisPrice"]').first().text().trim()
              
              let discount = ''
              $(el).find('*').each((_: number, textEl: any) => {
              ...
              let link = href ? (href.startsWith('http') ? href : `https://www.amazon.com${href}`) : ''

              const img = $(el).find('img').first().attr('src') || `https://img.usecurling.com/p/400/400?q=amazon%20product`

              if (title && title !== "Sem título" && link && !link.includes('javascript:')) {
                items.push({
                  raw_data: {
                    html_title: title,
                    ...
                    discount_rules: discount || (oldPrice ? `De ${oldPrice} por ${price}` : 'Desconto disponível'),
