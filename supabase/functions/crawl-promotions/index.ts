...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (targetUrl) {
      ...
      try {
        ...
        const isBlocked =
          ...
          pageTitle.includes('distil')

        if (isBlocked) {
          addLog(`Bloqueio anti-bot detectado`, {
            status: page.status,
            title: pageTitle,
          })
          throw new Error(
            `Acesso bloqueado pelo site de destino (Status HTTP: ${page.status}). Título da página: "${$('title').text()}". O site está utilizando proteção anti-bot que impede a leitura automatizada.`,
          )
        }
        ...
        const priceElements = $(
          '[class*="price" i], [id*="price" i], [data-price], [class*="discount" i], [class*="savings" i], [class*="savings" i], *:contains("You save"), *:contains("was $")',
        )
        let priceIndex = 1
        ...
        $('script[type="application/json"], script[id="__NEXT_DATA__"]').each(
          (i, el) => {
            try {
    ...
    if (items.length === 0) {
      if (!targetUrl) {
        throw new Error(
          'Nenhuma URL fornecida. A extração real exige uma URL específica.',
        )
      } else {
        throw new Error(
          'A página foi acessada, mas nenhum dado pôde ser extraído da estrutura HTML fornecida.',
        )
      }
    }
