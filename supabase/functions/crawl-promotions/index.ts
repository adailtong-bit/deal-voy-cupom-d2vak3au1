  ...
  try {
    ...
    if (targetUrl) {
      ...
      try {
        ...
        const isBlocked = page.status === 403 || page.status === 429 || pageTitle.includes('robot') || pageTitle.includes('verify') || pageTitle.includes('captcha') || pageTitle.includes('security') || pageTitle.includes('blocked');

        if (isBlocked) {
            throw new Error(`Acesso bloqueado pelo site de destino (Status: ${page.status}). O site está utilizando proteção anti-bot. Falha na extração real.`);
        }
        ...
        const priceElements = $('[class*="price" i], [id*="price" i], [data-price], [class*="discount" i], [class*="savings" i]');
        let priceIndex = 1;
