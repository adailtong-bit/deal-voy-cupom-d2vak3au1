  ...
  try {
    ...
    if (targetUrl) {
      ...
      try {
        ...
        if (isBlocked) {
          throw new Error(
            `Acesso bloqueado pelo site de destino (Status: ${page.status}). O site está utilizando proteção anti-bot. Falha na extração real.`,
          )
        }
        ...
        const priceElements = $(
          '[class*="price" i], [id*="price" i], [data-price], [class*="discount" i], [class*="savings" i]',
        )
        let priceIndex = 1
    ...
    if (items.length === 0) {
      if (!targetUrl) {
        ...
        throw new Error('Nenhum dado pôde ser extraído da URL fornecida.')
      }
    }
