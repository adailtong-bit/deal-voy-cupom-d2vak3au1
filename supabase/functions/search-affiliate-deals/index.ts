...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const mapped = ranked.slice(0, limit).map(r => ({
      ...
      imageUrl: r.image,
      productLink: r.link,
      storeName: r.title.includes('Amazon') ? 'Amazon' : r.title.includes('AliExpress') ? 'AliExpress' : 'Parceiro Afiliado',
      status: 'approved',
      category: 'affiliate',
      currency: 'USD',
    ...
    return new Response(JSON.stringify({ items: mapped, total: mapped.length }), {
