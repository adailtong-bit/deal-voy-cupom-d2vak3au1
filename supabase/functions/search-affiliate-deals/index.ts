...
async function fetchAffiliateDeals(query: string, affiliateIds: Record<string, string>) {
  ...
  const amazonId = affiliateIds?.amazon || affiliateIds?.Amazon || 'routevoy_amz'
  const aliexpressId = affiliateIds?.aliexpress || affiliateIds?.AliExpress || 'routevoy_ali'
  const shopeeId = affiliateIds?.shopee || affiliateIds?.Shopee || 'routevoy_shp'
